...
class ImageGenerator(object):
    '''
    图片生成器
    饱和度，亮度，照明，对比度，水平翻转和垂直翻转变换
    使训练模型有更强的适应性
    '''
    def __init__(self, ground_truth_data, batch_size, image_size, 
        train_keys, validation_keys,
        ground_truth_transformer=None, path_prefix=None,
        saturation_var=0.5, brightness_var=0.5, contrast_var=0.5, lighting_std=0.5,
        horizontal_flip_probability=0.5, vertical_flip_probability=0.5,
        do_random_crop=False, grayscale=False,
        zoom_range=[0.75, 1.25], translation_factor=.3):
                ...
                if mode == 'train' or mode == 'demo':
                    if self.ground_truth_transformer != None:
                        image_array, ground_truth = \
                            self.transform(image_array, ground_truth)
                        ground_truth = \
                            (self.ground_truth_transformer.assign_boxes(ground_truth))
                    else:
